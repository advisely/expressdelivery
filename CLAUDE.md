...
| `electron/menu.ts`         | Application menu bar builder (File/Edit/View/Message/Window/Help), `menu:action` IPC dispatch |
...
- **Phase 10 (v1.8.0):** Custom application menu bar (`electron/menu.ts`, File/Edit/View/Message/Window/Help), `menu:action` IPC channel with renderer dispatch, send countdown fix (single-interval, dismiss button), DevTools enabled in production, AI & Agentic README section, 646 tests across 27 files
